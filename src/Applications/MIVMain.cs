...
internal class MIVMain
{
    ...
    public static void printMIVScreen(char[] chars, int pos, string infoBar, bool editMode)
    {
        ...
        if (editMode) System.Console.Write(countNewLine + 1 + "," + countChars);
    }
    ...
    public static void mivEditor(string specifiedFile)
    {
        if (specifiedFile == "bf.mcf")
        {
            Methods.WriteLine("ERROR: This file is protected by the system and cannot be deleted.", ConsoleColor.Red);
            return;
        }

        string text = string.Empty;

        text = miv(File.ReadAllText(specifiedFile));

        System.Console.Clear();

        if (text != null)
        {
            File.WriteAllText(specifiedFile, text);
    ...
    public static void StartMIV(string specifiedFile)
    {
        try
        {
            if (File.Exists(specifiedFile))
            {
                System.Console.WriteLine("Found file!");
            }
            else if (!File.Exists(specifiedFile))
            {
                System.Console.WriteLine("Creating file!");
                File.Create(specifiedFile);
            }
